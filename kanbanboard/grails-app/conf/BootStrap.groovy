class BootStrap {
     ...
     def init = { servletContext ->
		...
		def secondStage = new StageRecord(user:"skye", kanban: secondKanban, stage: peerStage).save();
		def thirdStage = new StageRecord(user:"skye", kanban: thirdKanban, stage: readyStage).save();
		firstKanban.addToStages(firstStage).save()
		secondKanban.addToStages(secondStage).save();
		thirdKanban.addToStages(thirdStage).save();
     }
     def destroy = {
     }
} 